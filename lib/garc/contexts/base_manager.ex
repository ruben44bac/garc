defmodule Garc.Contexts.BaseManager do
  @moduledoc """
  ...
  Example:
    use Garc.Contexts.BaseManager, repo: Repo, schema: Schema
  ...
  defmacro __using__(opts) do
    quote do
      import Ecto.Query
      import Garc.Utils.Queries
      alias Garc.Contexts.BaseManager

      @schema unquote(Keyword.fetch!(opts, :schema))
      @repo unquote(Keyword.get(opts, :repo, Garc.Repo))
      ...
      @type paginate :: %{page: Integer.t(), page_size: Integer.t()} | nil
  ...
  defmacro generate_get_all do
    quote do
      ...
      """
      @spec get_all(filter_list(), order_by(), preload(), paginate()) :: list(@schema.t())
      def get_all(
            filters \\ [],
      ...
      def get_all(filters, order, preloads, nil) do
        BaseManager.do_get_all(
          {@repo, @schema},
          {filters, order, preloads}
        )
      end

      def get_all(filters, order, preloads, paginate) do
        BaseManager.do_get_all(
          {@repo, @schema},
          {filters, order, preloads, paginate}
        )
      end
  ...
  defmacro generate_create do
    quote do
      ...
      """
      @spec create(map(), list()) :: response_tuple()
  ...
  def do_get_all({repo, schema}, {filters, order, preloads}) do
    schema
    |> where([], ^filters)
    |> order_by(^order)
    |> preload(^preloads)
  ...
  def do_get_all({repo, schema}, {filters, order, preloads, %{page: page, page_size: page_size}}) do
    schema
    |> where([], ^filters)
    |> order_by(^order)
    |> preload(^preloads)
